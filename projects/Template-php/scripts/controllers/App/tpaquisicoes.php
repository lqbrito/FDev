<?php
	...
	class tpaquisicoesController extends Controller
	{
		...
		public function excluir()
		{
			...
			try
			{
				$tpaquisicoes = $this->model()->find($this->filterInput('id', 'int'));
				$this->model()->beginTransaction();
				$this->model()->delete();
				$this->model()->commit();
		...
		{
			if (isset($this->input))
				return ['tpaquisicoes' => $this->input];
			else
				return ['tpaquisicoes' => $this->clearFields()];
		}
		...
		{
			$tpaquisicoes = $this->model()->find($_POST['id']);
		...
		{
			parent::__construct();
			$this->model = new tpaquisicoesModel();
		}
	...
	$index = tpaquisicoesController::getInstance();
